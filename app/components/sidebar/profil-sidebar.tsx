...
const ProfilSidebarNav = ({ onClickAction }: {
  onClickAction: () => void;
}) => (
    ...
    <ul>
      {profilSidebarLinks.map((link) => (
        <li key={link.name} className="mb-4">
          {link.href ? (
            <Link href={link.href} className="flex justify-start gap-5 group font-manrope" onClick={() => onClickAction()}>
    ...
    <>
      <aside className={`
        ...
        <div className="sticky top-0 flex flex-col h-screen p-4 overflow-y-auto font-manrope">
          <ProfilSidebarNav onClickAction={() => setIsOpenSidebar(false)}/>
        </div>
      </aside>
      <div
